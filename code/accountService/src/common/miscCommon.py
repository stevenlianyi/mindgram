...
def setLogNew(title, filebasename, homeDir=r"../../log"):
    ...
    logfile = pathlib.Path(logfile).as_posix()
    logger = logging.getLogger(title)
    file_handler = logging.FileHandler(logfile)
    formatter = logging.Formatter('%(name)-12s %(asctime)s %(levelname)-8s %(message)s')
    file_handler.setFormatter(formatter)
...
def jsonDumps(data, ensure_ascii=True):
    if ensure_ascii:
...
def saveJsonData(fileName, dataList, ensure_ascii=True):
    if dataList!=None:
        typeStyle=type(dataList)
#        print (dataList)
        with open(fileName, "w", encoding='utf-8') as hFile:
            if typeStyle==dict:
                strT=jsonDumps(dataList, ensure_ascii)+"\n"
                hFile.write(strT)
            else:
                for a in dataList:
...
def write2CSV(fileName, dataList, method):
